...
@pytest.mark.asyncio
async def test_message_to_self(generate_access_token1, websocket_send_message):
    """
    test sending message to self
    """
    token, user = generate_access_token1
    message = "hello self"
    msg_response = await websocket_send_message(
        token=token, message=message, to=user.username, endpoint="/chat/dm/"
    )
...
async def test_send_msg_to_user2(
    generate_access_token1, generate_access_token2, websocket_send_message
):
    """
    ...
    msg_response1 = await websocket_send_message(
        token=token1, message=message1, to=user2.username, endpoint="/chat/dm/"
    )
    msg_response2 = await websocket_send_message(
        token=token2, message=message2, to=user1.username, endpoint="/chat/dm/"
    )
...
@pytest.mark.asyncio
@pytest.mark.django_db(transaction=True)
async def test_message_threads(generate_new_token, websocket_send_message):
    """
    ...
    msg_response1 = await websocket_send_message(
        token=token1, message=msg1, to=user2.username, endpoint="/chat/dm/"
    )
    msg_response2 = await websocket_send_message(
        token=token2, message=msg2, to=user1.username, endpoint="/chat/dm/"
    )
    ...
    await websocket_send_message(
        token=token3, message=msg3, to=user1.username, endpoint="/chat/dm/"
    )
